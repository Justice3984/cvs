...
1.  <details>
    ...
    > Length: 4, Capacity: 5
    - `arr[:4]` creates a new _slice_ using the first 4 elements of `arr`. `arr` can be an array (as it is here), or another slice.
...
1.  <details>
    ...
    5. The capacity therefore needs to be increased, and a capacity increase is _usually_ a doubling: 5 \* 2 = 10
...
1.  <details>
    ...
    - Because we created a _new_ slice with `make`, it is completely unrelated to the array, so setting a value in the slice has no effect on the array.
